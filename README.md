...
1. В личном чате с ботом отправьте ссылку.
2. После загрузки бот пришлет видео.
3. Команда `/settings` открывает настройки:
   - качество видео (`Лучшее`, `720p`, `480p`);
   - добавлять ссылку в подпись или нет;
   - отправлять видео как видео или как файл.
   - выбранные настройки сохраняются после перезапуска бота.
